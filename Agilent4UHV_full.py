#    "$Name:  $";
#    "$Header: /siciliarep/CVS/tango_ds/Vacuum/VacuumController/Agilent4UHV.py,v 1.3 2007/08/28 10:39:17 srubio Exp $";
#=============================================================================
#
# file :        Agilent4UHV.py
#
...
if 'PyDeviceClass' not in dir(PyTango):
...
#         This Device will manage the Agilent 4UHV as it was a Varian DUAL Ion Pump Controller.
#         It has been written to allow an in-place replacement of the former.
...
class Agilent4UHV(Dev4Tango, MultiGaugeProtocol):
    ...
    def is_Attr_allowed(self, req_type):
        ...
        r = bool(self.SVD 
                    and self.SVD.errors < len(self.SVD.readList) 
                    and self.get_state() not in [PyTango.DevState.UNKNOWN] 
                    and self.SVD.init)  # ,PyTango.DevState.INIT] )
        return r
    ...
    def WarmUp(self):
        ...
        e.g. ON1 = lambda: ON1()
        e.g. SERIAL = lambda: SERIAL()
        And for each element we will execute d['SERIAL'].__call__()
        As several commands are causing Controller Death ... ON1 and ON2 must be
        specified instead of using ON for All.
        '''
        ...
        self.StartSequence = filter(bool,(l.split('#')[0].strip() for l in self.StartSequence))
        
        if self.StartSequence:
            if len(self.StartSequence) == 1 and ',' in self.StartSequence:
                self.StartSequence = self.StartSequence[0].split(',')
                
            self.info('-' * 80)
            self.info('In WarmUp() = executeStartSequence (%s)' %
                      self.StartSequence)  
            for s in self.StartSequence:
                self.info(s)
                s, c = s.split(':', 1) if ':' in s else (s, '')
                try:
                    c = True if not c else fandango.TangoEval().eval(c)
                except:
                    c = False
                self.info('\t%s:%s' % (s, c))
    ...
    def SetStartSequence(self, argin):
        if argin:
            if argin.upper() == 'DELETE':
                self.StartSequence = []
            else:
                self.StartSequence = argin
            db = PyTango.Database()
            db.put_device_property(
                self.get_name(), {'StartSequence': self.StartSequence})
    ...
    def writeCommand(self, comm, argin, mode=False, local=False):
        ...
        print '>' * 80
        if mode:
            local = time.time() >= self.last_serial_change + \
                60 and self.ForceLocal
            if not 'SERIAL' in self.getModeLocal():
                self.SetMode('serial')
        else:
            local = False
        try:
    ...
    def readCommand(self, comm, _type):
        ...
        try:
            ...
            # OJORL! The 3 first characters are discarded (not 4). The initial
            # '>' is discarded by the SVD class.
            if result is not None:
                self.debug('readCommand(%s): Data(%d,%s) readed: "%s"' %
                           (comm, len(result) - 3, result, result[3:]))
                result = result[3:]
            ...
            if result is not None and len(result):
                if re.match(self.REGEXPS[_type], result):
                    ...
                    PyTango.Except.throw_exception(
                        'Agilent4UHV_CommFailed', 'Hardware read failed', 'Agilent4UHV.readCommand(%s,%s)=%s' % (comm, str(_type), result))
            else:
                PyTango.Except.throw_exception(
                    "Agilent4UHV_ValueNotUpdatedException", 'Last %s read was an empty string.' % str(comm), 'Agilent4UHV.readCommand(%s)' % str(comm))
            ...
            PyTango.Except.throw_exception(
                "Agilent4UHV_readCommand Exception", str(e), str(e))
            # PyTango.Except.re_throw_exception(e,"Agilent4UHV","read_"+comm+"()",result)
...
#------------------------------------------------------------------
    def __init__(self, cl, name):
        ...
        Agilent4UHV.init_device(self)

#------------------------------------------------------------------
#    Device destructor
#------------------------------------------------------------------
    def delete_device(self):
        self.info("[Device delete_device method] for device%s" %
                  self.get_name())
        if self.SVD:
            self.SVD.stop()
        del self.SVD
    ...
    def init_device(self):
        ...
        try:
            if not hasattr(self, 'LogLevel'):
                self.LogLevel = 'INFO'
            self.info(
                ''.join(("In ", self.get_name(), "::init_device(%s)" % self.LogLevel)))
            ...
            if not hasattr(self, 'Refresh') or not self.Refresh:
                ...
                self.Refresh = 5.
            self.info('Refresh pause between connections set to %s seconds.' %
                      self.Refresh)
            ...
            if not self.SerialLine:
                ...
                # raise RuntimeError, str('SerialLine property requires a
                # value!')
                self.SVD = None
            else:
                # The arguments for SerialVacuumDevice are:
                # tangoDevice=SerialLineName, period=minimum time between
                # communications, wait=time waiting for answer
                ...
                # READ COMMANDS #VARIAN DUAL PROTOCOL: MultiGauge Compatible
                # (No Checksum)
                addCommand('HV1 V', self.packMultiGauge(
                    1, self.GeneralComms['V Meas'], '?'), 1.)
                addCommand('HV1 I', self.packMultiGauge(
                    1, self.GeneralComms['I Meas'], '?'), 1.)
                addCommand('HV1 P', self.packMultiGauge(
                    1, self.GeneralComms['P Meas'], '?'), 1.)
                addCommand('HV2 V', self.packMultiGauge(
                    2, self.GeneralComms['V Meas'], '?'), 1.)
                addCommand('HV2 I', self.packMultiGauge(
                    2, self.GeneralComms['I Meas'], '?'), 1.)
                addCommand('HV2 P', self.packMultiGauge(
                    2, self.GeneralComms['P Meas'], '?'), 1.)

                addCommand('ModeLocal', self.packMultiGauge(
                    0, self.GeneralComms['Local/Remote'], '?'), 10.)
                addCommand('Remote Error', self.packMultiGauge(
                    0, self.GeneralComms['Remote Error'], '?'), 10.)
                addCommand('Interlock', self.packMultiGauge(
                    0, self.GeneralComms['Interlock'], '?'), 10.)
                addCommand('ErrorStatus', self.packMultiGauge(
                    1, self.GeneralComms['Error Status'], '?'), 10.)
                addCommand('HV1Status', self.packMultiGauge(
                    1, self.GeneralComms['HV On/Off'], '?'), 10.)
                addCommand('HV2Status', self.packMultiGauge(
                    2, self.GeneralComms['HV On/Off'], '?'), 10.)
                addCommand('HV1Step', self.packMultiGauge(
                    1, self.HighVoltageCommands['fixed/step'], '?'), 10.)
                addCommand('HV1Protect', self.packMultiGauge(
                    1, self.HighVoltageCommands['start/protect'], '?'), 10.)
                addCommand('HV2Step', self.packMultiGauge(
                    2, self.HighVoltageCommands['fixed/step'], '?'), 10.)
                addCommand('HV2Protect', self.packMultiGauge(
                    2, self.HighVoltageCommands['start/protect'], '?'), 10.)
                addCommand('HV1 IProtect', self.packMultiGauge(
                    1, self.HighVoltageCommands['Iprotect'], '?'), 10.)
                addCommand('HV2 IProtect', self.packMultiGauge(
                    2, self.HighVoltageCommands['Iprotect'], '?'), 10.)
                addCommand('HV1 PSetPoint', self.packMultiGauge(
                    1, self.HighVoltageCommands['SetPt1'], '?'), 10.)
                addCommand('HV2 PSetPoint', self.packMultiGauge(
                    2, self.HighVoltageCommands['SetPt1'], '?'), 10.)

                addCommand('Firmware', self.packMultiGauge(
                    0, self.GeneralComms['Firmware'], '?'), 60.)
                addCommand('Pump1', self.packMultiGauge(
                    1, self.GeneralComms['Device Type'], '?'), 60.)
                addCommand('Pump2', self.packMultiGauge(
                    2, self.GeneralComms['Device Type'], '?'), 60.)

                # WRITE COMMANDS
                # THESE COMMANDS MUST NOT BE ADDED TO THE COMMON COMMANDS LIST
                self.HVComms['Serial On'] = self.packMultiGauge(
                    0, self.GeneralComms['Local/Remote'], '2')
                self.HVComms['Serial Off'] = self.packMultiGauge(
                    0, self.GeneralComms['Local/Remote'], '0')
                self.HVComms['HV1 On'] = self.packMultiGauge(
                    1, self.GeneralComms['HV On/Off'], '1')
                self.HVComms['HV1 Off'] = self.packMultiGauge(
                    1, self.GeneralComms['HV On/Off'], '0')
                self.HVComms['HV2 On'] = self.packMultiGauge(
                    2, self.GeneralComms['HV On/Off'], '1')
                self.HVComms['HV2 Off'] = self.packMultiGauge(
                    2, self.GeneralComms['HV On/Off'], '0')
                self.HVComms['HV1setFixed'] = self.packMultiGauge(
                    1, self.HighVoltageCommands['fixed/step'], '0')
                self.HVComms['HV1setStep'] = self.packMultiGauge(
                    1, self.HighVoltageCommands['fixed/step'], '1')
                self.HVComms['HV1setStart'] = self.packMultiGauge(
                    1, self.HighVoltageCommands['start/protect'], '0')
                self.HVComms['HV1setProtect'] = self.packMultiGauge(
                    1, self.HighVoltageCommands['start/protect'], '1')
                self.HVComms['HV2setFixed'] = self.packMultiGauge(
                    2, self.HighVoltageCommands['fixed/step'], '0')
                self.HVComms['HV2setStep'] = self.packMultiGauge(
                    2, self.HighVoltageCommands['fixed/step'], '1')
                self.HVComms['HV2setStart'] = self.packMultiGauge(
                    2, self.HighVoltageCommands['start/protect'], '0')
                self.HVComms['HV2setProtect'] = self.packMultiGauge(
                    2, self.HighVoltageCommands['start/protect'], '1')

                self.SVD.period = max(
                    (self.Refresh, len(self.SVD.pollingList) * .1))

                self.SVD.start()  # self.SVD.updateThread.start()
                self.WarmUp()
        except Exception, e:
            self.error('Exception in Agilent4UHV.init_device(): ' + str(e))
            self.init_error = 'Exception in Agilent4UHV.init_device(): ' + str(
                e)
            print getLastException()
            PyTango.Except.throw_exception(
                "Agilent4UHV_initDeviceException", str(e), str(e))
            # PyTango.Except.re_throw_exception(e,"Agilent4UHV","read_"+comm+"()",result)
        ...
        try:
            if self.SerialLine and self.SVD:
                ...
                if self.SVD.init == False:  # If done in 2 lines to avoid changing to ON by default
                    state, channelstatus = DevState.INIT, 'Hardware values not read yet, started at %s' % time.ctime(
                        self.startTime)
                elif self.SVD.errors >= len(self.SVD.readList) or self.SVD.lasttime < time.time() - 2 * 60:
                    state, channelstatus = DevState.UNKNOWN, 'Unable to communicate with the device since %s' % time.ctime(
                        self.SVD.lasttime)
                ...
                # Checking interlock state through HV1/HV2 Status
                    ...
                    if self.HV1Status in wrongstates.keys():
                    ...
                    elif not any([s.lower() == 'on' for s in [self.HV1Status, self.HV2Status] if s]):
                        state = DevState.OFF
                        ...
                        channelstatus += 'Channel Status (%s,%s) differ from defaults (%s)' % (
                            self.HV1Status, self.HV2Status, self.DefaultStatus)
                    else:
                        state = DevState.ON
                ...
                if prev != state and self.statesQueue.index(state) is None:
                    ...
                    self.statesQueue.append(
                        state, 1 if state == DevState.ON else 20)

                # Getting next state to process (it will be actual state if
                # there's no new states in queue)
                state = self.statesQueue.pop()
                if state == DevState.MOVING:
                    channelstatus += self.oscillation
                else:
                    self.oscillation = ''

                self.comms_report = self.SVD.getReport()
                status = '\n'.join(
                    s for s in [channelstatus, self.Description, self.init_error, self.comms_report, '', self.exception.replace('\n', ''), ] if s)
                if state is None:
                    self.error('The StateQueue is EMPTY!!!')
                elif prev != state:
                    self.info('*' * 80)
                    self.info('%s.State changed from %s to %s' %
                              (self.get_name(), str(prev), str(state)))
                    self.info(status)
                    self.info('*' * 80)
...
#==================================================================
#
#    Agilent4UHV read/write attribute methods
#
#==================================================================
#------------------------------------------------------------------
...
#------------------------------------------------------------------
    ...
    def read_V1(self, attr):
        ...
        self.voltages[0][0], self.voltages[0][
            1] = self.voltages[0][1], attr_V1_read
...
#------------------------------------------------------------------
    ...
    def read_V2(self, attr):
        ...
        self.voltages[1][0], self.voltages[1][
            1] = self.voltages[1][1], attr_V2_read
...
#------------------------------------------------------------------
    ...
    def read_I1(self, attr):
        ...
        self.currents[0][0], self.currents[0][
            1] = self.currents[0][1], attr_I1_read
...
#------------------------------------------------------------------
    ...
    def read_I2(self, attr):
        ...
        self.currents[1][0], self.currents[1][
            1] = self.currents[1][1], attr_I2_read
...
#------------------------------------------------------------------
    def read_IonPumpsConfig(self, attr):
        ...
        #    Add your own code here
        last_read_time, last_read_value = getattr(
            self, '_last_IonPumpsConfig', (0, []))
        ...
        attr_IonPumpsConfig_read = [
            r.strip().replace(' ', '').lower() for r in result]
...
#------------------------------------------------------------------
    def write_IonPumpsConfig(self, attr):
        ...
        if len(data) != 2:
            PyTango.Except.throw_exception(
                'WrongDataLenght', 'Data length should be equal to NumOfChannels',
        ...
        for chan in (1, 2):
            ...
            if not numbers:
                PyTango.Except.throw_exception(
                    'UknownType', '%s not in IonPumpTypes list' % data, 'write_IonPumpsConfig')
            commands.append(
                self.packMultiGauge(chan, self.GeneralComms['Device Number'], chr(0x30 + numbers[0])))
        ...
        if any(a.strip() != self.ACK for a in answer.strip().split('\n')):
            PyTango.Except.throw_exception(
                'NACK', 'ACK not received: %s' % answer, 'write_IonPumpsConfig')
...
#------------------------------------------------------------------
    def read_IProtectSetPoints(self, attr):
        ...
        #    Add your own code here
        # setpoints = lambda d:','.join(s[3:] for s in
        # map(astor.proxies[d].SendCommand,['#171?\n\r','#172?\n\r']))
        ...
        attr.set_value(
            attr_IProtectSetPoint_read, len(attr_IProtectSetPoint_read))
...
#------------------------------------------------------------------
    def write_IProtectSetPoints(self, attr):
        ...
        if len(data) != 2:
            PyTango.Except.throw_exception(
                'WrongDataLenght', 'Data length should be equal to 2',
                'write_IProtectSetPoint')
        data = map(int, data)
        self.SendCommand([
            self.packMultiGauge(
                1, self.HighVoltageCommands['Iprotect'], '%05d' % data[0]),
            self.packMultiGauge(
                2, self.HighVoltageCommands['Iprotect'], '%05d' % data[1])
        ])
...
#------------------------------------------------------------------
    def read_PressureSetPoints(self, attr):
        ...
        attr_PressureSetPoints_read = [
            '%1.1e' % s for s in (protect1, protect2)]
        attr.set_value(
            attr_PressureSetPoints_read, len(attr_PressureSetPoints_read))
...
#------------------------------------------------------------------
    def write_PressureSetPoints(self, attr):
        ...
        self.SendCommand([
            ...
            self.packMultiGauge(
                1, self.HighVoltageCommands['SetPt1'], '%1.1e' % data[0]),
            self.packMultiGauge(
                2, self.HighVoltageCommands['SetPt1'], '%1.1e' % data[1])
        ])
    ...
    def read_ModeLocal(self, attr):
        ...
        try:
            if (time.time() > self.last_serial_change + 60
                ...
                self.info('>' * 80)
                self.info('ForceLocal=%s; Forcing LOCAL mode.' %
                          self.ForceLocal)
                self.SetMode('local')
        except:
    ...
    def read_ModeProtect(self, attr):
        ...
        try:
            if (time.time() > self.last_serial_change + 60
                ...
                self.info('>' * 80)
                self.info('ForceProtect=%s: Forcing PROTECT mode.' %
                          self.ForceProtect)
...
#------------------------------------------------------------------
    def read_ErrorStatus(self, attr=None):
        ...
        if aname == 'ErrorStatus':
            ...
            raise Exception('Error reading %s' % attr.get_name())
        if attr:
            attr.set_value(st)
        else:
            return self.DualControllerErrorStatus['HV'].get(str(st), '')
...
#------------------------------------------------------------------
    def read_Missreadings(self, attr=None, value=None):
        if not hasattr(self, 'missreadings'):
            setattr(self, 'missreadings', [])
        if value is not None and value not in self.missreadings:
            self.missreadings.append(str(value))
        if len(self.missreadings) > 256:
            self.missreadings = self.missreadings[
                -256:-1] + [self.missreadings[-1]]
        if attr is not None:
            attr.set_value(self.missreadings)
...
#------------------------------------------------------------------
    ...
    def SaveBlackBox(self, filename):
        ...
        if filename != '':
            self.SVD.blackbox.save(filename)
        return self.SVD.blackbox.to_string()

#==================================================================
#
#    Agilent4UHV command methods
#
#==================================================================
...
#------------------------------------------------------------------
    def SendCommand(self, argin, separator='', mode=True):
        ...
        try:
            ...
            PyTango.Except.throw_exception(
                'SendCommandError', str(e), 'SendCommand')
        self.SVD.start()
        if '!' in result:
            code = result[result.index('!') + 1]
            self.error('ProtocolError%s: %s' %
                       (code, self.ProtocolErrors[code]))
            # raise Exception('ProtocolError%s:
            # %s'%(code,self.ProtocolErrors[code]))
            PyTango.Except.throw_exception(
                'ProtocolError', self.ProtocolErrors[code], 'SendCommand')
        if mode and local:
            self.SetMode('local')
        return result
        ...
        if argin.lower() == 'serial':
            if time.time() >= self.last_serial_change + SERIAL_WAIT:
                ...
                while time.time() < self.last_serial_change + 1.:
                    self.event.wait(.05)
                    if 'SERIAL' in self.getModeLocal():
                        break
                if 'SERIAL' not in self.getModeLocal():
                    print self.warning('SetMode(SERIAL) failed?')
                    # raise Exception('SetMode(SERIAL) failed!')
                self.info('Mode changed to serial in %s seconds.' %
                          (time.time() - self.last_serial_change))
            else:
                self.info('\tSERIAL mode already set %d seconds ago.' %
                          (time.time() - self.last_serial_change))
            ...
            local = time.time() >= self.last_serial_change + \
                SERIAL_WAIT and self.ForceLocal
            self.SetMode('serial')
            if argin.lower() == 'fixed':
                ...
                self.writeCommand(
                    'HV1setProtect', self.HVComms['HV1setProtect'])
                self.writeCommand(
                    'HV2setProtect', self.HVComms['HV2setProtect'])
...
#==================================================================
#
#    Agilent4UHVClass class definition
#
#==================================================================


class Agilent4UHVClass(PyTango.PyDeviceClass):
    ...
    cmd_list = {
        ...
        'SetMode':
            [[PyTango.DevString, "Switches between configuration modes: LOCAL/SERIAL , STEP/FIXED , START/PROTECT"],
             [PyTango.DevString,
                "Switches between configuration modes: LOCAL/SERIAL , STEP/FIXED , START/PROTECT"],
             {
                'Display level': PyTango.DispLevel.EXPERT,
            }],
        'On':
            [[PyTango.DevVoid, "Switches On both High voltage channels (managed by DefaultStatus)"],
             [PyTango.DevString,
                "Switches On both High voltage channels (managed by DefaultStatus)"],
             {
                'Display level': PyTango.DispLevel.EXPERT,
            }],
        'Off':
            [[PyTango.DevVoid, "Switchs Off both High voltage channels (managed by DefaultStatus)"],
             [PyTango.DevString,
                "Switchs Off both High voltage channels (managed by DefaultStatus)"],
             {
                'Display level': PyTango.DispLevel.EXPERT,
            }],
        'OnHV1':
        ...
        'AsciiChecksum':
            [[PyTango.DevString, "Returns the valid checksum for an Ascii Command"],
             [PyTango.DevString,
                "Returns the valid checksum for an Ascii Command"],
             {'Display level': PyTango.DispLevel.EXPERT, }],
        'SetStartSequence':
            [[PyTango.DevVarStringArray, "Set StartSequence, empty to see current, DELETE to erase it"],
             [PyTango.DevVarStringArray,
                "Set StartSequence, empty to see current, DELETE to erase it"],
             {'Display level': PyTango.DispLevel.EXPERT, }],
        'SaveBlackBox':
    ...
    attr_list = {
        ...
        'HV2Code':
            [[PyTango.DevShort, PyTango.SCALAR, PyTango.READ],
                {'Display level': PyTango.DispLevel.EXPERT, }],
        'Missreadings':
            [[PyTango.DevString, PyTango.SPECTRUM, PyTango.READ, 256],
                {'Display Level': PyTango.DispLevel.EXPERT, }],
        'BlackBox':
...
for k in Agilent4UHVClass.attr_list.keys():
    comm = 'is_%s_allowed' % k
    if not hasattr(Agilent4UHV, comm):
        setattr(Agilent4UHV, comm, Agilent4UHV.is_Attr_allowed)


#==================================================================
#
#    Agilent4UHV class main method
#
#==================================================================
