        ...
        try:
            ...
            if result is not None and len(result):
                if re.match(self.REGEXPS[_type],result):
                    ...
                    PyTango.Except.throw_exception('Agilent4UHV_CommFailed','Hardware read failed','Agilent4UHV.readCommand(%s,%s)=%s'%(comm,str(_type),result))
            else:
                PyTango.Except.throw_exception("Agilent4UHV_ValueNotUpdatedException",'Last %s read was an empty string.'%str(comm),'Agilent4UHV.readCommand(%s)'%str(comm))
        
        except Exception, e:
            self.error('Exception in readCommand: '+str(e))
            #print getLastException()
            self.exception='\nLast exception: '+str(e)
            PyTango.Except.throw_exception("Agilent4UHV_readCommand Exception",str(e),str(e))    
...
#------------------------------------------------------------------
    def init_device(self):
        ...
        try:
            ...
            self.init_error='Exception in Agilent4UHV.init_device(): '+str(e)
            print getLastException()
            PyTango.Except.throw_exception("Agilent4UHV_initDeviceException",str(e),str(e))    
            #PyTango.Except.re_throw_exception(e,"Agilent4UHV","read_"+comm+"()",result)
